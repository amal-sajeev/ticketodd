"""
PR&DW Grievance Portal — One-Click Runner
==========================================
...
def setup_env():
    ...
    lines = [
        ...
        f"# Qdrant is optional — leave blank to run without knowledge base",
        f"QDRANT_URL=",
        f"QDRANT_API_KEY=",
    ]
    ENV_PATH.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def check_mongodb():
    ...
    try:
        ...
        print(f"    • Local install:  mongod --dbpath /data/db")
        print(f"    • Docker:         docker run -d -p 27017:27017 --name mongo mongo:7")
        print(f"    • Or set MONGODB_URL in .env to point to your instance\n")
        sys.exit(1)


# ── Step 4: Seed database (MongoDB only, no Qdrant) ─────────────────────────
def seed_database():
